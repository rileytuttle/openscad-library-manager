 ...
 * 4. retainer (boolean)
 */
module skadis_u_holder(d = 16, all_pegs = all_pegs, fullfill = fullfill, retainer = retainer, thickness) {
    thick = thickness == undef ? pw : thickness;
    union() {
        difference() {
            translate([0, -(3*thick+d)/2, thick/2]) {
                cube(size = [d+2*thick, 3*thick+d, thick], center = true);
            }
            hull() {
                translate([0, -d/2-2*thick, -extra]) {
                    cylinder(h = thick + extra, d = d, center = false);
                }
                translate([0, -(3*thick+d), -extra]) {
                    cylinder(h = thick + extra, d = d-8*lh, center = false);
                }
            }
            hull() {
                translate([0, -d/2-2*thick, thick-chamfer() + extra]) {
                    cylinder(h = chamfer() + extra, d1 = d, d2 = d+8*lh, center = false);
                }
                translate([0, -(3*thick+d), thick-chamfer()]) {
 ...
 * 6. retainer (boolean)
 */
module skadis_plier(l = distance_between_pegs+pt-2*pw, w = 2*distance_between_pegs/5, filet = chamfer(), all_pegs = all_pegs, fullfill = fullfill, retainer = retainer, h=pw, thickness=pw) {
    union() {
        difference() {
            hull() {
                translate([0, -(pw/2), h/2]) {
                    cube(size = [l+2*thickness, pw, h], center = true);
                }
                for (x = [0:1:1]) {
                    mirror([x, 0, 0]) {
                        translate([l/2-filet_limits(l, w, filet), -(w-filet_limits(l, w, filet)+2*thickness), 0]) {
                            cylinder(h = h, r = filet_limits(l, w, filet)+thickness);
                        }
                    }
                }
            }
            translate([0, -(w/2+2*thickness), 0]) {
                hull() for (y = [0:1:1]) {
                    mirror([0, y, 0]) for (x = [0:1:1]) {
                        mirror([x, 0, 0]) {
                            translate([l/2-filet_limits(l, w, filet), w/2-filet_limits(l, w, filet), -extra]) {
                                cylinder(h = h + extra, r = filet_limits(l, w, filet));
                            }
                        }
                    }
                }
            ...
            hull() {
                translate([0, -(w/2+2*thickness), 0]) {
                    hull() for (y = [0:1:1]) {
                        mirror([0, y, 0]) for (x = [0:1:1]) {
                            mirror([x, 0, 0]) {
                                translate([l/2-filet_limits(l, w, filet), w/2-filet_limits(l, w, filet), h-chamfer()]) {
                ...
                translate([0, -(w/2+2*thickness), 0]) {
                    hull() for (y = [0:1:1]) {
                        mirror([0, y, 0]) for (x = [0:1:1]) {
                            mirror([x, 0, 0]) {
                                translate([l/2-filet_limits(l, w, filet), w/2-filet_limits(l, w, filet), h]) {
